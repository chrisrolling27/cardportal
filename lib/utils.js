...
export function extractDetail(endpoint, response) {
  ...
  if (endpoint.includes("paymentMethods")) {
    const methods = Array.isArray(response.paymentMethods) ? response.paymentMethods.length : 0;
    const stored = Array.isArray(response.storedPaymentMethods) ? response.storedPaymentMethods.length : 0;
    if (methods || stored) {
      const parts = [];
      if (methods) parts.push(`${methods} method${methods === 1 ? "" : "s"}`);
      if (stored) parts.push(`${stored} stored`);
      return `Available payment methods: ${parts.join(", ")}`;
    }
    return "Fetched payment methods";
  }
